...
# from src.models.extra_model.SwinTransformer_mona import ChannelVarianceSorter
...
class SwinTransformer_mona(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_heads = [3, 6, 12, 24],
                 ##################base##########################
                 # embed_dim = 128,  # 从96改为128
                 # depths = [2, 2, 18, 2],  # 从[2, 2, 6, 2]改为[2, 2, 18, 2]
                 # num_heads = [4, 8, 16, 32],  # 从[3, 6, 12, 24]改为[4, 8, 16, 32]
                 #################small#########################
                 # embed_dim=96,
                 # depths = [2, 2, 18, 2],
                 # num_heads = [3, 6, 12, 24],
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # 过滤出Swin相关的权重（排除Mona模块的权重）
            swin_state_dict = {}
            for k, v in new_state_dict.items():
                # 只保留不包含'my_module'的键（即Swin原始权重）
                if 'my_module' not in k:
                    swin_state_dict[k] = v
            ...
            # 更新模型字典，只更新Swin部分的权重
            model_dict.update(swin_state_dict)

            # 加载到模型中
            msg = self.load_state_dict(model_dict, strict=False)
            logger.info(f'加载预训练权重: {msg}')

            # 打印加载信息
            logger.info(f'成功加载 {len(swin_state_dict)} 个Swin参数')
            logger.info(f'Mona模块参数将保持随机初始化')
        ...
        for name, param in self.named_parameters():
            if 'my_module' not in name:
                ...
                param.requires_grad = True  # Mona模块可训练
