"""
Implementation of various mathematical operations in the Poincare ball models of hyperbolic space. Some
...
# 在 hyptorch/pmath.py 中替换原有 RiemannianGradient 类
class RiemannianGradient(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, c):
        """
        前向传播：保存x和c到上下文，确保c与x同设备
        x: 输入张量（已在目标设备，如cuda:0）
        c: 曲率参数（张量，必须与x同设备）
        """
        # 强制c与x同设备
        c = c.to(x.device)
        ctx.save_for_backward(x, c)  # 保存x和c（后续反向传播用）
        return x  # 前向传播输出x

    @staticmethod
    def backward(ctx, grad_output):
        """反向传播：使用保存的x和c计算梯度缩放因子"""
        x, c = ctx.saved_tensors  # 从上下文恢复x和c（已同设备）
        # 计算双曲空间梯度缩放（所有运算在同一设备）
        scale = (1 - c * x.pow(2).sum(dim=-1, keepdim=True)).pow(2) / 4
        return grad_output * scale, None  # 仅返回x的梯度（c的梯度忽略）
...
def expmap(x, u, *, c=1.0):
    r"""
    Exponential map for Poincare ball models. This is tightly related with :func:`geodesic`.
    ...
    For the Poincare ball models the solution is well defined globally and we have.
    .. math::
...
def expmap0(u, *, c=1.0):
    r"""
    Exponential map for Poincare ball models from :math:`0`.
    .. math::
...
def poincare_mean(x, dim=0, c=1.0):
    # 首先投影到 Klein coordinate，计算出中心点，然后将中心点投影回Poincare ball
    x = p2k(x, c)
...
def _dist_matrix_pyclustering(x, y, c):
    # x = torch.tensor(x)
    # y = torch.tensor(y)
